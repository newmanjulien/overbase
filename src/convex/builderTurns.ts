...
import { generateBuilderTurnResult, generateEmailPolishTurnResult } from './model';
...
export const generateEmailPolishTurn = internalAction({
	args: {
		conversationId: v.id('conversations'),
		assistantMessageId: v.id('messages'),
		generationId: v.string()
	...
	handler: async (ctx, { conversationId, assistantMessageId, generationId }) => {
		...
		try {
			...
			const turn = await generateEmailPolishTurnResult({
				transcript,
				draft: builderContext.emailDraft,
				artifactVersion: builderContext.artifactVersion
			});

			await ctx.runMutation(internal.builderSessions.completePolishTurn, {
				assistantMessageId,
				generationId,
				turn
			});

			console.log('builderTurns.generateEmailPolishTurn completed', {
				conversationId,
				assistantMessageId,
				generationId,
				totalDurationMs: Date.now() - startedAt,
			...
			const message = error instanceof Error ? error.message : 'AI email polish failed.';

			await ctx.runMutation(internal.chat.failAssistantMessage, {
				assistantMessageId,
				generationId,
				text: 'I could not polish the email draft from that edit. Please try again.',
				errorText: message
			});

			console.error('builderTurns.generateEmailPolishTurn failed', {
				conversationId,
				assistantMessageId,
				generationId,
				totalDurationMs: Date.now() - startedAt,
