...
export function getCreateFormatGalleryCategoryFromSearchParams(
	...
	const hasExplicitCategoryFilter = searchParams.has('mode') || searchParams.has('industry');
	...
	const selectedCategory =
		CREATE_FORMAT_GALLERY_CATEGORIES.find(
			(category) => category.mode === selectedMode && category.industry === selectedIndustry
		) ?? null;
	...
	return getCreateFormatGalleryCategoryById(fallbackCategoryId) ?? CREATE_FORMAT_GALLERY_CATEGORIES[0];
}
...
function getIndustryCategoryInfoBar(industryTagId: FormatStarterIndustryTagId) {
	switch (industryTagId) {
		case 'law':
			return {
				label: 'Tip:',
				text: 'Select a starter to design an email format that your team will receive'
			};
		case 'insurance':
			return {
				label: 'Tip:',
