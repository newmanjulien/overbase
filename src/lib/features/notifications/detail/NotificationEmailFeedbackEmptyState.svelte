<script lang="ts">
	import { MessageCircle } from 'lucide-svelte';
</script>

<section class="flex h-full min-h-0 items-center justify-center bg-zinc-50/50 px-5 py-8">
	<div class="flex max-w-60 flex-col items-center text-center">
		<MessageCircle aria-hidden="true" class="mb-4 size-5 text-zinc-950" />

		<h2 class="text-[0.78rem] leading-tight font-medium text-zinc-950">
			No emails sent yet
		</h2>

		<p class="mt-2 text-[0.67rem] leading-relaxed text-zinc-600">
			Once this notification sends emails, you can give feedback on specific emails and train the AI on what to improve.
		</p>
	</div>
</section>
