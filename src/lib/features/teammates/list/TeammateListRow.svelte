...
<div class="grid min-h-9 min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-x-3">
	<span class="block min-w-0">
		...
		{#if item.role}
			<span class="block truncate text-[0.68rem] text-zinc-400">{item.role}</span>
		{/if}
	</span>
	{#if !item.isEditing}
		<Button
		...
		<div class="grid gap-2">
			<label class="min-w-0 text-[0.68rem] text-zinc-700">
				...
				<input
					...
					class="h-8 w-full rounded-sm border border-zinc-200/70 bg-white px-2.5 text-[0.74rem] text-zinc-900 outline-none transition-colors placeholder:text-zinc-400 focus:border-zinc-400"
				/>
			</label>
