import { buildFormatsGalleryHref } from '$lib/app/app-links';
import {
	builderParagraph as paragraph,
	builderText as text,
	builderVariable as variable
} from '$lib/features/builder/domain';
import { defineBuilder, seededSpreadsheetAttachment, spreadsheetCell as cell } from './helpers';
...
export const reconnectLinkedinBuilder = defineBuilder({
	mode: 'public-data',
	...
	startingPointSelection: {
		kind: 'guided',
		intro:
			"Lets build an email format that helps your team easily reconnect with their LinkedIn contacts. We'll take LinkedIn contacts then find news that gives an easy reason to reconnect",
		infoBar: {
			label: 'Tip:',
			content: [
				{
	...
	ruleInfoCard: {
		...
		content: [
			{
				kind: 'text',
				text: 'Everything is preset because this builder only uses public data. Builders that '
			},
			{
				kind: 'link',
				label: 'use internal data',
				href: buildFormatsGalleryHref('internal-data')
			},
			{
				kind: 'text',
