<script lang="ts">
	...
	import type { BuilderVariableDragCoordinator } from './builder-variable-drag-coordinator.svelte';
	...
	function startDrag(event: DragEvent, variable: BuilderVariableDefinition) {
		dragCoordinator.startPaletteDrag(event, {
			...variable,
			label: formatBuilderVariableLabel(variables, variable.id)
		});
	}
</script>
	...
	<div class="mt-4 rounded-sm border border-stone-200/50 bg-white p-3.5">
		<div class="flex flex-wrap gap-2">
			{#each variables as variable (variable.id)}
				...
				>
					{formatBuilderVariableLabel(variables, variable.id)}
				</button>
			{/each}
		</div>
	</div>
