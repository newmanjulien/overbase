<script lang="ts">
	...
	type Props = {
		setup: BuilderSetup;
		onAnswersChange: (answers: BuilderSetupAnswers) => void;
		onSubmit: (answers: BuilderSetupAnswers) => void;
	};
	...
	function materializeSkippedAnswers(fromQuestionIndex = 0) {
		...
		for (let index = fromQuestionIndex; index < setup.questions.length; index += 1) {
			...
			if (questionId && !answers[questionId]) {
				answers[questionId] = BUILDER_SETUP_SKIPPED_ANSWER;
			}
		}
			...
			<div class="mt-6 rounded-sm border border-stone-100 bg-white p-4 md:mt-8 md:p-6">
				<div class="flex items-start justify-between gap-4">
					<div class="flex min-w-0 items-center gap-1.5">
						...
						<HelpTooltip
							id={`builder-setup-question-help-${currentQuestion.id}`}
