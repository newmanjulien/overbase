...
export type BuilderInlineDragItem<Context extends object> =
...
export type BuilderInlineControllerAdapter<Context extends object> = {
	scope: string;
...
type BuilderSurfaceActionParams<Context extends object> = {
	context: Context;
...
export class BuilderInlineController<Context extends object> {
	selectedVariable = $state<BuilderInlineVariableIdentity<Context> | null>(null);

	private readonly adapter: BuilderInlineControllerAdapter<Context>;
	private caretBookmark: BuilderCaretBookmark | null = null;
	private handledInsertionRequestId = 0;

	constructor(adapter: BuilderInlineControllerAdapter<Context>) {
	...
	private surfaceAction = (
		...
		return {
			update: (nextParams: BuilderSurfaceActionParams<Context>) => {
	...
	handleDocumentPointerDown = (event: PointerEvent) => {
		...
		if (
			...
			target.closest('[data-builder-inline-controller-variable]')
		) {
			return;
		}
	...
	private configureSurface(element: HTMLElement, context: Context) {
		element.dataset.builderInlineSurfaceKey = this.adapter.contextKey(context);
	}
	...
	private serializeInlineNodes(parent: Node): BuilderInlineNode[] {
	...
	private createVariablePill(
		variableId: string,
		source: BuilderInlineVariableIdentity<Context>
	) {
		const pill = createVariablePill({
			variableId,
			label: formatBuilderVariableLabel(this.adapter.variables(), variableId),
			source,
		...
		pill.dataset.builderInlineControllerVariable = 'true';
		return pill;
	}
	...
	private selectVariable = (source: BuilderInlineVariableIdentity<Context>) => {
		if (this.adapter.disabled()) {
			return;
		}
	...
	private deleteSelectedVariable = (source: BuilderInlineVariableIdentity<Context>) => {
	...
	private startVariableDrag = (
		...
		const label = formatBuilderVariableLabel(this.adapter.variables(), variableId);
		const item: BuilderInlineDragItem<Context> = {
			...source,
			variableId,
			sourcePill,
			sourceSurface
	...
	moveVariableToTarget(item: unknown, target: BuilderVariableDropTarget | null) {
	...
	insertVariableAtTarget(target: BuilderVariableDropTarget | null, variableId: string) {
		if (
	...
	private getVariableTapInsertionTarget(): BuilderVariableDropTarget | null {
	...
	private getBookmarkedInsertionTarget(): BuilderVariableDropTarget | null {
	...
	private closestSurface(node: Node | null) {
		...
		const surface = element?.closest('[data-builder-inline-surface-key]');
		const root = this.adapter.root();

		return surface instanceof HTMLElement && root?.contains(surface) ? surface : null;
	}

	private resolveInlineDragItem(item: unknown): BuilderInlineDragItem<Context> | null {
		if (
			typeof item !== 'object' ||
			item === null ||
			!('variableId' in item) ||
		...
		const candidate = item as Partial<BuilderInlineDragItem<Context>>;
		...
		return candidate as BuilderInlineDragItem<Context>;
	}
}
