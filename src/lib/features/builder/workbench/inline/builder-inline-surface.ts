import type { BuilderInlineNode } from '$lib/features/builder/domain';

export type BuilderInlineSurfaceOptions = {
	variableDatasetKey: string;
	normalize?: (nodes: readonly BuilderInlineNode[]) => BuilderInlineNode[];
	joinMode: 'body' | 'cell';
};

export type BuilderVariablePillOptions<Source> = {
	variableId: string;
	label: string;
	source: Source;
	selected: boolean;
...
export type BuilderInlineInsertionResult = {
	nodes: BuilderInlineNode[];
...
export function cloneBuilderInlineNodes(nodes: readonly BuilderInlineNode[]) {
	return nodes.map((node) => ({ ...node }));
}

export function normalizeSurfaceNodes(nodes: readonly BuilderInlineNode[]) {
...
export function areInlineNodesEqual(
	left: readonly BuilderInlineNode[],
	right: readonly BuilderInlineNode[]
) {
...
export function createVariablePill<Source>({
	...
	onDragStart,
	onDragEnd
}: BuilderVariablePillOptions<Source>) {
	...
	pill.className =
		'builder-variable-pill inline-flex items-center rounded-full border border-transparent bg-emerald-50/80 px-1.5 py-[0.125rem] align-baseline text-[0.73rem] font-normal leading-none text-emerald-900' +
		(selected ? ' builder-variable-pill--selected' : '');
...
export function renderVariablePillContents(
	...
	labelElement.className = 'builder-variable-pill__label';
	labelElement.textContent = label;

	const removeButton = document.createElement('button');
	removeButton.type = 'button';
	removeButton.className = 'builder-variable-pill__remove';
...
export function joinSegments(
	beforeNodes: readonly BuilderInlineNode[],
	middleNodes: readonly BuilderInlineNode[],
	afterNodes: readonly BuilderInlineNode[],
	options: BuilderInlineSurfaceOptions
) {
	const before = normalizeInlineNodes(cloneBuilderInlineNodes(beforeNodes), options);
	const middle = normalizeInlineNodes(cloneBuilderInlineNodes(middleNodes), options);
	const after = normalizeInlineNodes(cloneBuilderInlineNodes(afterNodes), options);
	const joined: BuilderInlineNode[] = [];

	function appendPart(part: BuilderInlineNode[]) {
		if (part.length === 0) {
			return;
		}
...
function joinSegmentsWithInsertedVariable(
	beforeNodes: readonly BuilderInlineNode[],
	variableNode: BuilderInlineNode,
	afterNodes: readonly BuilderInlineNode[],
	options: BuilderInlineSurfaceOptions
): BuilderInlineInsertionResult {
	const before = normalizeInlineNodes(cloneBuilderInlineNodes(beforeNodes), options);
	...
	function appendPart(part: BuilderInlineNode[], isInsertedVariable = false) {
		if (part.length === 0) {
			return;
		}
...
export function removeChild(
	...
	const beforeNodes: BuilderInlineNode[] = [];
	const afterNodes: BuilderInlineNode[] = [];
...
export function insertVariable(
	...
	const variableNode: BuilderInlineNode = { type: 'variable', variableId };
...
function trimTrailingHorizontalSpace(nodes: BuilderInlineNode[]) {
...
function trimLeadingHorizontalSpace(nodes: BuilderInlineNode[]) {
	const first = nodes[0];
...
function startsWithVariable(nodes: readonly BuilderInlineNode[]) {
	return nodes[0]?.type === 'variable';
}

function endsWithSentenceEndingPunctuation(nodes: readonly BuilderInlineNode[]) {
	const last = nodes.at(-1);

	return last?.type === 'text' && SENTENCE_ENDING_PUNCTUATION.has(last.text.at(-1) ?? '');
}

function startsWithTightFollowingPunctuation(nodes: readonly BuilderInlineNode[]) {
	const first = nodes[0];
