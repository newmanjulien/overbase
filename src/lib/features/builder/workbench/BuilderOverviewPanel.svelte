<script lang="ts">
	...
	import type { BuilderRegistryEntry } from '../../../../builders/registry';
	import BuilderPanelArtwork from './BuilderPanelArtwork.svelte';
	...
	const artwork = $derived(toBuilderArtworkPreset(builder.artwork).panel);
	...
	<div class="flex min-h-0 flex-1 items-center justify-center px-5 py-10 md:px-8">
		<article class="w-full max-w-66 rounded-sm bg-white p-4 text-stone-950 shadow-sm">
			...
			<div class="px-1 pt-1 pb-5 text-center">
				<h2 class="text-xs font-medium tracking-normal text-stone-950">{builder.title}</h2>
				<p class="mt-1 text-[0.7rem] leading-snug text-stone-500">{builder.description}</p>
				...
				>
					<div class="min-h-0 overflow-hidden">
						...
						>
							{#each builder.details.paragraphs as paragraph (paragraph)}
								<p>{paragraph}</p>
							{/each}
						</div>
					</div>
