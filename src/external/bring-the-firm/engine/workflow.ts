...
function buildRefinementUserPrompt(params: {
	draft: EmailDraft;
	recentEvents: BringTheFirmRefinementParams['recentEvents'];
}) {
	return [
...
async function streamEmailDraftRefinementTurn(params: {
	systemPrompt: string;
	userPrompt: string;
	transcript: BringTheFirmRefinementParams['transcript'];
	draft: EmailDraft;
	handlers: BringTheFirmRefinementParams['handlers'];
...
export async function createBringTheFirmInitialDraft(params: BringTheFirmInitialDraftParams) {
	const prompt = buildBringTheFirmInitialDraftPrompt(params);
