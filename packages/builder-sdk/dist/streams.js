...
function getStreamErrorMessage(event) {
    return event.error?.message ?? event.response?.error?.message;
}
function throwIfStreamFailed(event) {
...
async function processSseChunk(chunk, onEvent) {
...
export async function readOpenAIStream(response, handlers) {
    let streamedText = '';
    let finalText = '';
...
function getOrCreateToolCall(toolCalls, event) {
...
function parseEmailDraftPatch(argumentsText) {
    if (!argumentsText.trim()) {
        throw new Error('OpenAI called update_email_draft without arguments.');
    }
    return JSON.parse(argumentsText);
}
export async function readEmailBuilderTurnStream(response, handlers) {
